...
void Shift::_seek(Head& h, float lag)
{
...
float Shift::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift::sinc_phases) p = shift::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift::sinc_table[p].data();
    const float* k1 = shift::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift::sinc_taps; j++)
    {
...
float Shift::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
