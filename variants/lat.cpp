...
void Shift_lat::_seek(Head& h, float lag)
{
...
float Shift_lat::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift_lat::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift_lat::sinc_phases) p = shift_lat::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift_lat::sinc_table[p].data();
    const float* k1 = shift_lat::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift_lat::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift_lat::sinc_taps; j++)
    {
...
void Shift_lat::_update_interval()
{
...
int32_t Shift_lat::_splice_coarse(uint32_t ref, int32_t sign)
{
...
float Shift_lat::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift_lat::_start_fade(float target_lag, float length, bool match_level)
{
...
void Shift_lat::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
...
template<> bool ShiftAdapter<Shift_lat>::set_param(const std::string& name, double value)
{
    const float v = static_cast<float>(value);
    if(name == "default_grain")        { Shift_lat::default_grain = v;        return true; }
    if(name == "fallback_corr_window") { Shift_lat::fallback_corr_window = v; return true; }
    if(name == "blind_span_cap")       { Shift_lat::blind_span_cap = v;       return true; }
    if(name == "onset_runway")         { Shift_lat::onset_runway = v;         return true; }
    if(name == "onset_grain")          { Shift_lat::onset_grain = v;          return true; }
    if(name == "guard_samples")        { Shift_lat::guard_samples = std::max(v, 17.f); return true; }
    if(name == "min_grain")            { Shift_lat::min_grain = v;            return true; }
    if(name == "yin_per_block")        { Shift_lat::yin_per_block = static_cast<size_t>(std::max(v, 1.f)); return true; }
    if(name == "max_corr_window")      { Shift_lat::max_corr_window = v;      return true; }
    if(name == "tracked_corr_min")     { Shift_lat::tracked_corr_min = v;     return true; }
    if(name == "exact_ratio")          { Shift_lat::exact_ratio = v > 0.5f;   return true; }
    return false;
}

#include "variant.hpp"
static RegisterShifter reg_lat{"lat", [] { return std::make_unique<ShiftAdapter<Shift_lat>>(); }};
