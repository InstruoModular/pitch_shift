...
void Shift_blind_margin::_seek(Head& h, float lag)
{
...
void Shift_blind_margin::_write(float x)
{
...
float Shift_blind_margin::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift_blind_margin::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift_blind_margin::sinc_phases) p = shift_blind_margin::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift_blind_margin::sinc_table[p].data();
    const float* k1 = shift_blind_margin::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift_blind_margin::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift_blind_margin::sinc_taps; j++)
    {
...
void Shift_blind_margin::_update_interval()
{
...
void Shift_blind_margin::_update_grain()
{
    ...
    if(ratio < 1.f)
    {
        lag_lo += (period_valid ? tairm::max(p * 0.125f, 12.f) : 0.35f * grain /* E3a: full blind reach */)
...
void Shift_blind_margin::_pitch_tick()
{
    if(!yin_running)
    {
...
void Shift_blind_margin::_yin_finalise()
{
...
int32_t Shift_blind_margin::_splice_coarse(uint32_t ref, int32_t sign)
{
...
float Shift_blind_margin::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift_blind_margin::_start_fade(float target_lag, float length)
{
...
void Shift_blind_margin::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
...
#include "variant.hpp"
static RegisterShifter reg_blind_margin{"blind_margin", [] { return std::make_unique<ShiftAdapter<Shift_blind_margin>>(); }};
