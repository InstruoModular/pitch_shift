...
void Shift_lat_causal::_seek(Head& h, float lag)
{
...
void Shift_lat_causal::_write(float x)
{
...
float Shift_lat_causal::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift_lat_causal::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift_lat_causal::sinc_phases) p = shift_lat_causal::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift_lat_causal::sinc_table[p].data();
    const float* k1 = shift_lat_causal::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift_lat_causal::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift_lat_causal::sinc_taps; j++)
    {
...
void Shift_lat_causal::_update_interval()
{
...
void Shift_lat_causal::_update_grain()
{
...
void Shift_lat_causal::_pitch_tick()
{
    if(!yin_running)
    {
...
void Shift_lat_causal::_yin_finalise()
{
...
int32_t Shift_lat_causal::_splice_coarse(uint32_t ref, int32_t sign)
{
...
float Shift_lat_causal::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift_lat_causal::_start_fade(float target_lag, float length, bool match_level)
{
...
void Shift_lat_causal::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
...
template<> bool ShiftAdapter<Shift_lat_causal>::set_param(const std::string& name, double value)
{
    const float v = static_cast<float>(value);
    if(name == "default_grain")        { Shift_lat_causal::default_grain = v;        return true; }
    if(name == "fallback_corr_window") { Shift_lat_causal::fallback_corr_window = v; return true; }
    if(name == "blind_span_cap")       { Shift_lat_causal::blind_span_cap = v;       return true; }
    if(name == "onset_runway")         { Shift_lat_causal::onset_runway = v;         return true; }
    if(name == "onset_grain")          { Shift_lat_causal::onset_grain = v;          return true; }
    if(name == "guard_samples")        { Shift_lat_causal::guard_samples = std::max(v, 17.f); return true; }
    if(name == "min_grain")            { Shift_lat_causal::min_grain = v;            return true; }
    if(name == "yin_per_block")        { Shift_lat_causal::yin_per_block = static_cast<size_t>(std::max(v, 1.f)); return true; }
    if(name == "max_corr_window")      { Shift_lat_causal::max_corr_window = v;      return true; }
...
#include "variant.hpp"
static RegisterShifter reg_lat_causal{"lat_causal", [] { return std::make_unique<ShiftAdapter<Shift_lat_causal>>(); }};
