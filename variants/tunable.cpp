...
void Shift_tunable::_seek(Head& h, float lag)
{
...
void Shift_tunable::_write(float x)
{
...
float Shift_tunable::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift_tunable::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift_tunable::sinc_phases) p = shift_tunable::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift_tunable::sinc_table[p].data();
    const float* k1 = shift_tunable::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift_tunable::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift_tunable::sinc_taps; j++)
    {
...
void Shift_tunable::_update_interval()
{
...
void Shift_tunable::_update_grain()
{
...
void Shift_tunable::_pitch_tick()
{
    if(!yin_running)
    {
...
void Shift_tunable::_yin_finalise()
{
...
int32_t Shift_tunable::_splice_coarse(uint32_t ref, int32_t sign)
{
...
float Shift_tunable::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift_tunable::_start_fade(float target_lag, float length, bool match_level)
{
...
void Shift_tunable::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
...
template<> bool ShiftAdapter<Shift_tunable>::set_param(const std::string& name, double value)
{
    const float v = static_cast<float>(value);
    if(name == "default_grain")        { Shift_tunable::default_grain = v;        return true; }
    if(name == "fallback_corr_window") { Shift_tunable::fallback_corr_window = v; return true; }
    if(name == "blind_span_cap")       { Shift_tunable::blind_span_cap = v;       return true; }
    return false;
}

static RegisterShifter reg_tunable{"tunable", [] { return std::make_unique<ShiftAdapter<Shift_tunable>>(); }};
