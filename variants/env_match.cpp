...
void Shift_env_match::_seek(Head& h, float lag)
{
...
void Shift_env_match::_write(float x)
{
...
float Shift_env_match::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift_env_match::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift_env_match::sinc_phases) p = shift_env_match::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift_env_match::sinc_table[p].data();
    const float* k1 = shift_env_match::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift_env_match::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift_env_match::sinc_taps; j++)
    {
...
void Shift_env_match::_update_interval()
{
...
void Shift_env_match::_update_grain()
{
...
void Shift_env_match::_pitch_tick()
{
    if(!yin_running)
    {
...
void Shift_env_match::_yin_finalise()
{
...
int32_t Shift_env_match::_splice_coarse(uint32_t ref, int32_t sign)
{
...
float Shift_env_match::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift_env_match::_start_fade(float target_lag, float length, bool match_level)
{
    ...
    if(match_level)
    {
        auto energy = [this](const Head& h)
        {
            float e = 1.0e-9f;
            for(int32_t j = -gain_window / 2; j < gain_window / 2; j++)
            {
        ...
        const float gm = std::sqrt(energy(head[active]) / energy(next));
...
void Shift_env_match::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
...
#include "variant.hpp"
static RegisterShifter reg_env_match{"env_match", [] { return std::make_unique<ShiftAdapter<Shift_env_match>>(); }};
