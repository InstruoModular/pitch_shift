#ifndef SHIFT_SMOOTH_HPP
#define SHIFT_SMOOTH_HPP
...
{
    public:
        template<class> friend struct ShiftAdapter;   // E8: set_param reaches the tunable statics
        Shift_smooth() { reset(); }
        ~Shift_smooth() = default;
    ...
    private:
        ...
        static inline float max_corr_window = 256.f;   /* lat: runtime-tunable */
        ...
        static inline float fallback_corr_window = 512.f;   /* E8: runtime-tunable */

        static inline float guard_samples = 40.f;   /* lat: runtime-tunable (sinc reach needs >= 17) */
        ...
        static inline float xfade_frac = 0.25f;
        static inline bool  period_median3 = false;   /* smooth: median of the last 3 raw YIN estimates */
        static inline float period_jump = 0.06f;      /* smooth: relative change treated as a new note */
        ...
        static inline float default_grain = 1536.f;   /* E8: runtime-tunable */ /* E3b: was 768 */   // when no pitch is found
        static inline float    onset_grain      = 768.f;   /* lat: runtime-tunable */   // E5: blind grain for the first 200 ms after an onset
         ...
         * not. */
        static inline float fine_corr_window = 64.f;   /* smooth: runtime-tunable */
