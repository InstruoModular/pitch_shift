...
void Shift_long_blind::_seek(Head& h, float lag)
{
...
void Shift_long_blind::_write(float x)
{
...
float Shift_long_blind::_read(const Head& h) const
{
    const float    ph = h.frac * static_cast<float>(shift_long_blind::sinc_phases);
    uint32_t       p  = static_cast<uint32_t>(ph);
    if(p >= shift_long_blind::sinc_phases) p = shift_long_blind::sinc_phases - 1u;
    const float    pf = ph - static_cast<float>(p);

    const float* k0 = shift_long_blind::sinc_table[p].data();
    const float* k1 = shift_long_blind::sinc_table[p + 1u].data();

    const uint32_t base = h.pos - static_cast<uint32_t>(shift_long_blind::sinc_centre);

    float acc = 0.f;
    for(size_t j = 0; j < shift_long_blind::sinc_taps; j++)
    {
...
void Shift_long_blind::_update_interval()
{
...
void Shift_long_blind::_update_grain()
{
...
void Shift_long_blind::_pitch_tick()
{
    if(!yin_running)
    {
...
void Shift_long_blind::_yin_finalise()
{
...
int32_t Shift_long_blind::_splice_coarse(uint32_t ref, int32_t sign)
{
...
float Shift_long_blind::_splice_fine(uint32_t ref, int32_t sign, int32_t best_dd) const
{
...
void Shift_long_blind::_start_fade(float target_lag, float length)
{
...
void Shift_long_blind::process(const MonoDspBuffer& input, MonoDspBuffer& output)
{
    _update_interval();
    _update_grain();
...
#include "variant.hpp"
static RegisterShifter reg_long_blind{"long_blind", [] { return std::make_unique<ShiftAdapter<Shift_long_blind>>(); }};
